...
def stripe_webhook(request):
    ...
    endpoint_secret = settings.STRIPE_WEBHOOK_SECRET  # Set this in your .env and Render
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        amount = session['amount_total'] / 100  # Stripe sends amount in cents
        email = session.get('customer_email', '')
        reference = session['id']

        # Save the donation
        Donation.objects.create(
            user=None,  # Or look up user by email if you want
            amount=amount,
            reference=reference,
            email=email,
            message=''
